...
Reinforcement Learning" (DeepMind, 2013), Section 4/5, PLUS three
stability improvements adopted from the 2015 Nature DQN follow-up:
a separate frozen target network, Huber (smooth-L1) loss instead of
MSE, and gradient-norm clipping. These reduce the loss/avg-max-Q
instability documented in DQN_Training_Guide.md Step 8, so the agent
converges more reliably in fewer episodes. `dqn_train_backup.py` is an
unmodified snapshot of the original, pure-Algorithm-1 version (no
target network, MSE loss, no clipping) kept for comparison.
...
# ---------------------------------------------------------------------------
NUM_EPISODES = 80 if not USE_TOY_ENV else 400
...
TARGET_SYNC_STEPS = 500 if not USE_TOY_ENV else 200
                               # how often (in env steps) to copy the live
                               # network's weights into the frozen target
                               # network (2015 Nature paper's C). Nature
                               # uses C=10,000 over a 1,000,000-frame decay;
                               # scaled down here by roughly the same ~50x
                               # factor already used for REPLAY_CAPACITY /
                               # EPS_DECAY_STEPS in this reduced-scale demo.
GRAD_CLIP_NORM = 10.0          # cap the gradient's L2 norm before each
                               # optimizer step -- standard DQN-literature
                               # default, bounds how much a single large
                               # Bellman error can move the weights.
...
def train(num_episodes: int = NUM_EPISODES, resume_weights_path: str | None = None):
    ...
    # Target network: a second copy of the network, frozen except for
    # periodic syncs every TARGET_SYNC_STEPS. Bootstrapping the Bellman
    # target off a network that only changes occasionally (instead of the
    # same, constantly-updating live network) breaks the feedback loop
    # where an overestimated Q-value inflates its own next target -- see
    # DQN_Training_Guide.md Step 8. Starts as an exact copy of net,
    # including any resumed weights loaded above.
    target_net = DQN(in_channels=4, num_actions=num_actions)
    target_net.load_state_dict(net.state_dict())
    target_net.eval()

    optimizer = optim.RMSprop(net.parameters(), lr=LEARNING_RATE)
    loss_fn = nn.SmoothL1Loss()  # Huber loss: quadratic near 0, linear for
                                 # large errors, so a big Bellman error
                                 # produces a bounded gradient instead of a
                                 # squared one (2015 Nature paper's choice).
    ...
    for episode in range(1, num_episodes + 1):
        ...
        while not done:
            ...
            if len(replay) >= max(MIN_REPLAY_BEFORE_TRAIN, BATCH_SIZE):
                ...
                #   y_j = r_j                              if terminal
                #   y_j = r_j + gamma * max_a' Q(s', a'; theta_target)   otherwise
                # DEVIATION from the 2013 paper's Algorithm 1 (documented,
                # deliberate): the original algorithm bootstraps max_a' off
                # the SAME network being updated, merely detached via
                # no_grad() for one backward pass -- theta is still "fixed"
                # for only a single step before the next update moves it
                # again. Here we use target_net, which is genuinely frozen
                # across TARGET_SYNC_STEPS env steps (2015 Nature paper's
                # fix). See dqn_train_backup.py for the original version.
                with torch.no_grad():
